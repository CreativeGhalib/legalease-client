...
describe('Phase 6 dashboard route registry', () => {
  it('exposes only the frozen role-specific navigation paths', () => {
    expect(dashboardRouteRegistry.user.map((item) => item.to)).toEqual([
      '/dashboard', '/dashboard/user/hiring-history', '/dashboard/user/update-profile', '/dashboard/user/comments', '/dashboard/user/transactions', '/dashboard/phone-verification', '/dashboard/sessions',
    ])
    expect(dashboardRouteRegistry.lawyer.map((item) => item.to)).toEqual([
      '/dashboard', '/dashboard/lawyer/hiring-history', '/dashboard/lawyer/manage-legal-profile', '/dashboard/lawyer/transactions', '/dashboard/lawyer/analytics', '/dashboard/phone-verification', '/dashboard/sessions',
    ])
