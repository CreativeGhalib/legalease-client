    ...
    return (
      <AuthPageLayout title="Choose your account type" subtitle="One last step: select how you will use LegalEase.">
        <form className="mt-6 space-y-5" onSubmit={handleSubmit(finishGoogleOnboarding)} noValidate>
          ...
          <fieldset>
            <legend className="text-sm font-medium text-slate-800 dark:text-[#ece5d6]">I am joining as</legend>
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <label className="rounded-lg border border-slate-200 dark:border-[#1c3050] p-4 text-sm text-slate-800 dark:text-[#ece5d6]"><input type="radio" value="user" {...register('role')} /> <span className="ml-2 font-medium">Client</span></label>
          ...
          <button disabled={isSubmitting} type="submit" className="min-h-11 w-full rounded-md bg-[#1b3a6b] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#142e57] dark:bg-[#d4a843] dark:text-[#0c1827] dark:hover:bg-[#e8bf58] disabled:cursor-not-allowed disabled:opacity-60">
    ...
    <AuthPageLayout title="Create your account" subtitle="Choose a client or lawyer account to get started.">
      <form className="mt-6 space-y-4" onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <label className="block text-sm font-medium text-slate-800 dark:text-[#ece5d6]">
          Full name
          <input autoComplete="name" aria-invalid={Boolean(errors.fullName)} className="mt-1 min-h-11 w-full rounded-md border border-slate-300 dark:border-[#1c3050] px-3 py-2 text-slate-950 dark:text-[#ece5d6] outline-none transition focus:border-slate-900 focus:ring-2 focus:ring-slate-200" {...register('fullName', { required: 'Full name is required.', minLength: { value: 2, message: 'Use at least 2 characters.' } })} />
          {errors.fullName && <span className="mt-1 block text-sm text-red-700 dark:text-red-300">{errors.fullName.message}</span>}
        </label>
        <label className="block text-sm font-medium text-slate-800 dark:text-[#ece5d6]">
          Email
        ...
        <fieldset>
          <legend className="text-sm font-medium text-slate-800 dark:text-[#ece5d6]">I am joining as</legend>
          <div className="mt-2 flex gap-4 text-sm text-slate-700 dark:text-[#ece5d6]">
      ...
      <p className="mt-5 text-sm text-slate-600 dark:text-[#a8bbcc]">Already have an account? <Link to="/login" state={{ from: location.state?.from }} className="font-medium text-slate-900 dark:text-[#ece5d6] underline">Sign in</Link></p>
    </AuthPageLayout>
  )
}
