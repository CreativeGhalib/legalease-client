  ...
  return (
    <div ref={rootRef} className="relative w-full">
      <button
        ...
        className="theme-select flex min-h-11 w-full items-center justify-between rounded-lg border border-[#243d66] bg-[#0f1e33] px-3 pr-10 text-left text-sm text-[#ece5d6] outline-none transition focus:border-[#c09a4e] dark:border-[#c5b89e] dark:bg-[#ece5d6] dark:text-[#0c1827]"
      >
      ...
      {open && (
        <div role="listbox" className="absolute z-30 mt-2 w-full overflow-hidden rounded-xl border border-[#1c3050] bg-[#0c1728] shadow-xl dark:border-[#d8ccb8] dark:bg-[#fdf9f2]">
          {options.map((option) => {
            ...
            return (
              <button
                ...
                className={`flex w-full items-center justify-between px-3 py-2.5 text-left text-sm transition ${active ? 'bg-[#c09a4e]/10 text-[#c09a4e] dark:bg-[#e8eef8] dark:text-[#1b3a6b]' : 'text-[#a8bbcc] hover:bg-[#132540] dark:text-[#364358] dark:hover:bg-[#e5dccf]'}`}
              >
...
export default function BrowseLawyersPage() {
  ...
  return (
    <section>
      <p className="text-sm font-semibold tracking-[0.16em] text-[#c09a4e] dark:text-[#1b3a6b]">PUBLIC DIRECTORY</p>
      <div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-semibold text-[#ece5d6] dark:text-[#0c1827] sm:text-4xl">Browse legal professionals</h1><p className="mt-2 max-w-2xl text-[#a8bbcc] dark:text-[#364358]">Search verified, published lawyer profiles by expertise, availability, and consultation fee.</p></div>{result && <p className="text-sm text-[#a8bbcc] dark:text-[#364358]">{result.meta.totalItems} professional{result.meta.totalItems === 1 ? '' : 's'} found</p>}</div>
      <div className="mt-8 rounded-2xl border border-[#1c3050] bg-[#0c1728] p-4 shadow-[0_10px_24px_rgba(7,16,31,0.18)] dark:border-[#d8ccb8] dark:bg-[#fdf9f2] sm:p-5">
        <div className="flex items-center gap-2 text-sm font-semibold text-[#ece5d6] dark:text-[#0c1827]"><SlidersHorizontal size={17} />Search &amp; filters</div>
        <div className="mt-4 grid gap-3 lg:grid-cols-6">
          <label className="relative lg:col-span-2"><span className="sr-only">Search lawyers</span><Search className="pointer-events-none absolute left-3 top-3 text-[#6b84a0] dark:text-[#69798e]" size={18} /><input value={searchText} onChange={(event) => setSearchText(event.target.value)} className="theme-input min-h-11 w-full rounded-lg border border-[#243d66] bg-[#0f1e33] py-2 pl-10 pr-3 text-sm text-[#ece5d6] placeholder:text-[#6b84a0] focus:border-[#c09a4e] focus:outline-none dark:border-[#c5b89e] dark:bg-[#ece5d6] dark:text-[#0c1827] dark:placeholder:text-[#69798e]" placeholder="Name or specialization" /></label>
          ...
          <label><span className="sr-only">Minimum consultation fee</span><input value={values.minFee} onChange={(event) => updateParams({ minFee: event.target.value, page: '1' })} type="number" min="0" step="0.01" className="theme-input min-h-11 w-full rounded-lg border border-[#243d66] bg-[#0f1e33] px-3 text-sm text-[#ece5d6] placeholder:text-[#6b84a0] focus:border-[#c09a4e] focus:outline-none dark:border-[#c5b89e] dark:bg-[#ece5d6] dark:text-[#0c1827] dark:placeholder:text-[#69798e]" placeholder="Min fee" /></label>
          <label><span className="sr-only">Maximum consultation fee</span><input value={values.maxFee} onChange={(event) => updateParams({ maxFee: event.target.value, page: '1' })} type="number" min="0" step="0.01" className="theme-input min-h-11 w-full rounded-lg border border-[#243d66] bg-[#0f1e33] px-3 text-sm text-[#ece5d6] placeholder:text-[#6b84a0] focus:border-[#c09a4e] focus:outline-none dark:border-[#c5b89e] dark:bg-[#ece5d6] dark:text-[#0c1827] dark:placeholder:text-[#69798e]" placeholder="Max fee" /></label>
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2 text-sm text-[#a8bbcc] dark:text-[#364358]"><span>Sort</span><div className="min-w-[12rem]"><FilterSelect label="Sort" value={values.sort} placeholder="Newest" onChange={(value) => updateParams({ sort: value, page: '1' })} options={[{ label: 'Newest', value: 'newest' }, { label: 'Fee: Low to High', value: 'fee-low' }, { label: 'Fee: High to Low', value: 'fee-high' }, { label: 'Most Hired', value: 'most-hired' }]} /></div></div>{hasFilters && <button type="button" onClick={() => { setSearchText(''); setSearchParams(new URLSearchParams()) }} className="min-h-10 text-sm font-semibold text-[#c09a4e] hover:text-[#d4af6a] dark:text-[#1b3a6b] dark:hover:text-[#0f2547]">Clear filters</button>}</div>
      </div>
